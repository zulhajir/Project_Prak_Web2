...
@section('header', 'Edit Konten')

@section('content')
    <div class="bg-white p-6 rounded-lg shadow-md max-w-3xl mx-auto">
        <h1 class="text-2xl font-bold mb-6 text-gray-800">Edit Konten: {{ $content->title }}</h1>
        ...
        <form action="{{ route('admin.contents.update', $content->id) }}" method="POST">
            ...
            <div class="mb-4">
                ...
                <select id="type" name="type" required class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" onchange="toggleContentTypeFields()">
                    ...
                    <option value="activity_report" {{ old('type', $content->type) == 'activity_report' ? 'selected' : '' }}>Laporan Kegiatan</option>
                    <option value="financial_report" {{ old('type', $content->type) == 'financial_report' ? 'selected' : '' }}>Laporan Keuangan</option>
                    <option value="impact_report" {{ old('type', $content->type) == 'impact_report' ? 'selected' : '' }}>Laporan Dampak</option>
                    <option value="volunteer_report" {{ old('type', $content->type) == 'volunteer_report' ? 'selected' : '' }}>Laporan Sukarelawan</option>
                    <option value="about_page" {{ old('type', $content->type) == 'about_page' ? 'selected' : '' }}>Halaman Tentang Kami</option>
                    <option value="contact_page" {{ old('type', $content->type) == 'contact_page' ? 'selected' : '' }}>Halaman Kontak Kami</option>
                </select>
            </div>
            ...
            <div id="common_fields">
                <div class="mb-4">
                    ...
                    <input type="text" id="title" name="title" value="{{ old('title', $content->title) }}" required class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline">
                </div>
                <div class="mb-4">
                    <label for="body" class="block text-gray-700 text-sm font-bold mb-2">Isi Konten/Laporan:</label>
                    <textarea id="body" name="body" rows="8" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline">{{ old('body', $content->body) }}</textarea>
                </div>
            </div>
            ...
            <div id="news_announcement_fields" class="hidden">
                <div class="mb-4">
                    ...
                    <textarea id="excerpt" name="excerpt" rows="3" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline">{{ old('excerpt', $content->excerpt) }}</textarea>
                </div>
                <div class="mb-4">
                    <label for="image_url" class="block text-gray-700 text-sm font-bold mb-2">URL Gambar Sampul (untuk Berita/Pengumuman):</label>
                    <input type="url" id="image_url" name="image_url" value="{{ old('image_url', $content->image_url) }}" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline">
                </div>
                <div class="mb-4">
                    <label for="is_published" class="inline-flex items-center">
                        <input type="checkbox" name="is_published" id="is_published" value="1" {{ old('is_published', $content->is_published) ? 'checked' : '' }} class="form-checkbox">
                ...
                <div class="mb-4">
                    ...
                    <select id="activity_id" name="activity_id" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline">
                        ...
                        @foreach($activities as $activity)
                            <option value="{{ $activity->id }}" {{ old('activity_id', $content->activity_id) == $activity->id ? 'selected' : '' }}>{{ $activity->title }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="mb-4">
                    <label for="file_url" class="block text-gray-700 text-sm font-bold mb-2">URL/Path File Laporan (Opsional):</label>
                    <input type="url" id="file_url" name="file_url" value="{{ old('file_url', $content->file_url) }}" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline">
                </div>
                <div class="mb-4">
                    <label for="report_date" class="block text-gray-700 text-sm font-bold mb-2">Tanggal Laporan:</label>
                    <input type="datetime-local" id="report_date" name="report_date" value="{{ old('report_date', $content->report_date ? \Carbon\Carbon::parse($content->report_date)->format('Y-m-d\TH:i') : '') }}" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline">
                </div>
            </div>
