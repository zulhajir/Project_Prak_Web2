...
@section('header', 'Tambah Donasi Baru')

@section('content')
    <div class="bg-white p-6 rounded-lg shadow-md max-w-3xl mx-auto">
        <h1 class="text-2xl font-bold mb-6 text-gray-800">Tambah Donasi Baru</h1>
        ...
        <form action="{{ route('admin.donations.store') }}" method="POST">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                ...
                <div>
                    ...
                    <input type="text" id="currency" name="currency" value="{{ old('currency', 'IDR') }}" required maxlength="3"
                           class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline">
                </div>
            </div>
            <div class="mb-4">
                <label for="donation_date" class="block text-gray-700 text-sm font-bold mb-2">Tanggal Donasi:</label>
                <input type="datetime-local" id="donation_date" name="donation_date" value="{{ old('donation_date', now()->format('Y-m-d\TH:i')) }}" required
                       class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline">
            </div>
            <div class="mb-4">
                <label for="payment_method" class="block text-gray-700 text-sm font-bold mb-2">Metode Pembayaran:</label>
                <input type="text" id="payment_method" name="payment_method" value="{{ old('payment_method') }}" required
                       class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline">
            </div>
            <div class="mb-4">
